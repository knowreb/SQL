...
--Jakie auta zostały wypożyczone? (stworzyc zapytanie, ktore zlaczy dwie tabele: dane_wypozyczen i samochody za pomoca where lub join) nr wypo marka i model
...
--dodajmy do wynikow informacje o klientach ktore dane wypozyczenie realizowali
SELECT 
	dane_wypozyczen.id_wypozyczenia,
    	samochody.marka,
    	samochody.model,
...
--wydobyc date wypozyczenia i jakie auto zostalo wypozyczone model i marka 
--(trzeba wykorzystac tablice znajdujaca sie pomiedzy tab wypozyczenia i samochody tj. tablica dane wypozyczen aby uzyskac dane o marce, modelu i dacie )
SELECT
	wypozyczenia.data_wyp,
    	samochody.marka,
    	samochody.model
...
-- zapytanie ktore zwroci nam nazwiska pracownikow oraz id wypozyczen
SELECT
	pracownicy.nazwisko_pracownika,
...
--2  Wyswietl marki i modele aut wraz z cenami za dobe wypozyczenia.
SELECT DISTINCT
	samochody.marka,
    	samochody.model,
...
--3 Wyswietl imiona i nazwiska pracownikow wypozyczani, ktorzy obslugiwali wypozyczenia w lipcu 2022 roku.
SELECT DISTINCT
	p.imie_pracownika,
    	p.nazwisko_pracownika
...
--4 Wyswietl marki i modele aut, ktore w okresie od kwietnia do wrzesnia wypozyczal klient o nazwisku Karwowski.
SELECT 
	s.marka,
    	s.model,
    	w.data_wyp,
...
--5 Wyswietl wartosc wypozyczen, ktore obslugiwal pracownik o nazwisku Nowacki. 
SELECT
